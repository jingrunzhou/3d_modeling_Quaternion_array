...
class Camera_Principles():
    ...
    def test_count(self,):
        ...
        test_Word = []
        test_Word.append([0.0,0,z0_test])
        test_Word.append([z0_test,0,z0_test])
        test_Word.append([0,z0_test,z0_test])
        test_Word.append([z0_test,z0_test,z0_test])
        # test_Word.append([100.0,100,z0_test])
        # test_Word.append([100.0,100,100])
        Pans = [0,45]
        Tilts = [0,45]
        for Pan in Pans:
            for Tilt in Tilts:
                ...
                test_RT = self.R_Rt(self.RT_count_ori(Tilt, Pan))
                test_4d = self.World_pixel(test_Word, test_RT, self.K, flag_test=True)

                for i in range(len(test_4d)):
                    print(f'Pan:{Pan},Tilt:{Tilt},原值:{test_Word[i]},转为:{test_4d[i]}')
    ...
    def count_no_img(self, Tilt, Pan,wPixel_real, hPixel_real):
        ...
        if False:#ok
            if False:
                ...
                self.RT_support = self.R_Rt(
                    self.RT_count_ori(self._T_constant_support + self._T_constant_camera, 0) @ self.RT_count_ori(Tilt,Pan))
            # 这两种方法的计算结果大致一样（几乎一样）
        elif False:
        ...
        if False:
            ...
            points_plate_fold = self.World_pixel(self.plates_fold_Word, self.RT, self.K, )
            # 不需要转
    ...
    def calculate_error(self, ):
        ...
        for name in os.listdir(self.root_path):
            ...
            np_predict = np.load(os.path.join(self.root_path, name.split('.')[0] + '_predict_solo.npy'))
