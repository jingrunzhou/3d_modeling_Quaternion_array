...
class Camera_Principles():
    def __init__(self, ID=None, root_path=None):
        ...
        self.flag_test = False  # 调试模式开关  如果不想画图，不想测试，该值为False,
        self.wPixel_referenc = 704
        self.hPixel_referenc = 576
        self.flag_vstack = True
        self.flag_Draw_disconnect = False
        self.flag_Draw_real_palte = False
        self.flag_Display_Width = False
        self.flag_Number_marked_points = True

        if False:  # 51
            self.fx, self.fy = [468.61, 680.34]  # [593.74,658.35]#[542.9,629.46]
            self.dist = np.array([-4.24103449e-01, 2.11013612e-01, -1.43886286e-03, -3.49882761e-05, -5.33531165e-02],
                                 dtype=np.float32)
        else:  # all
            self.fx, self.fy = [468.64, 680.222]
            self.dist = np.array([-4.23709796e-01, 2.23283964e-01, -6.57576014e-04, -4.92667899e-05, -6.80752617e-02],
                                 dtype=np.float32)
        self.k1, self.k2, self.p1, self.p2, self.k3 = self.dist

        if os.path.exists(self.name_initial_guess) and self.flag_test:
            self.d_level, self.installHeight, self.transmissionBelt_Coal_wall1, self._T_constant_camera, self._T_constant_support = np.load(self.name_initial_guess, allow_pickle=True)
    ...
    def init_jr(self):
        ...
        self.K_count()
        self.print_jr(self.flag_calculation_depends, self.d_level, self.installHeight, self.transmissionBelt_Coal_wall1,
    ...
    def mark_lines(self, points_all, img_mark, num_line_each_group=4):
        ...
        lines4 = np.array([points_all[n] for n in range(len(points_all)) if
                           points_num_each_line * 3 <= n % points_num_each_group and self.point_cheak(points_all[n],
                                                                                                      self.wPixel_real,
                                                                                                      self.hPixel_real,
                                                                                                      0.5)]).astype(
            np.int32)
        # 中间有停顿的画法，能把间断单独显示
        if self.flag_Draw_disconnect:
                ...
                if True:
                    if right_point_num == 0:
                        ...
                        if self.flag_Draw_real_palte:
                            ...
                            # 标记点的序号
                            if self.flag_Number_marked_points:
                            ...
                            if True:
                                ...
                                if self.point_cheak(points):
                                    for i in range(len(points) - 1):
                                        ...
                                        if self.point_cheak(ph21, self.wPixel_real,
                                                            self.hPixel_real) and self.point_cheak(ph22,
                                                                                                   self.wPixel_real,
                                                                                                   self.hPixel_real):
                        ...
                        if True:
                            ...
                            if (top_left is not None) and (bottom_right is not None) and j - i >= 1:
                                ...
                                cv2.putText(img_mark, text, (text_x, text_y), font, font_scale, (0, 255, 0),
                                            font_thickness)

                        # 画真实的护帮板,画到底板，算损失用
                        if True:
                            points_transmissionBelts = np.array([point for point in points if
                                                                 self.point_cheak(point, self.wPixel_real,
                                                                                  self.hPixel_real, 0.5)]).astype(
                                np.int32)
                            if len(points_transmissionBelts) > 2:
                                ...
                                cv2.drawContours(self.count_like_label, [points_transmissionBelts], -1, (0, 255, 85),
                                                 -1)
            ...
            if 'all' not in self.flag_calculation_depends:
                ...
                for calculation_npt_depend in calculation_npt_depends:  # calculation_npt_depend  的区域算损失
                    ...
                    self.count_like_label_nps[
                        self.count_like_label_nps == self.list_class.index(calculation_npt_depend)] = 0
            else:
                pass
            # 使用 == 运算符比较两个数组
    ...
    def run_jr(self, Tilt, Pan, image1, mat_intri, coff_dis):
        ...
        if True:
            ...
            distCoeffs = coff_dis.astype(
                np.float32)  # 畸变系数，一个 1x5、1x8 或 1x12 的数组。这些系数表示径向畸变（k1, k2, k3）和切向畸变（p1, p2, k4, k5, k6）。如果为 None，则不考虑畸变。
    ...
    def minimize_count(self):
        ...
        result = minimize(self.error_function, initial_guess, method='COBYLA', bounds=bounds,
